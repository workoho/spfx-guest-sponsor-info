...
/** Builds the Bash one-liner that bootstraps PowerShell and runs the installer. */
function buildShellInstallCommand(semver: string | undefined): string {
  const url = buildScriptUrl(semver, 'install.sh');
...
interface IWelcomeDialogProps {
   ...
   * installer URLs in step 2. Falls back to the latest release URL when
   * undefined.
   */
  semver?: string;
...
const Step2Setup: React.FC<IStep2SetupProps> = ({
  ...
  const [commandMode, setCommandMode] = React.useState<'powershell' | 'shell'>('powershell');
  const installCmd = React.useMemo(
    () => commandMode === 'shell' ? buildShellInstallCommand(semver) : buildInstallCommand(semver),
    [commandMode, semver]
  );
  const installTokens = React.useMemo(
    () => commandMode === 'powershell' ? tokenizePwsh(installCmd) : [],
    [commandMode, installCmd]
  );
  const installUrl = React.useMemo(
    () => buildScriptUrl(semver, commandMode === 'shell' ? 'install.sh' : 'install.ps1'),
    [commandMode, semver]
  );
  const commandLabel = commandMode === 'shell' ? strings.WelcomeDialogSetupShellHint : strings.WelcomeDialogSetupPwshHint;
  const commandSwitchLabel = commandMode === 'shell'
    ? strings.WelcomeDialogUsePowerShellCommandLabel
    : strings.WelcomeDialogUseShellCommandLabel;
      ...
      <RadioGroup value={choice} onChange={(_, d) => onChoiceChange(d.value as 'api' | 'demo')}>
        ...
        <div className={mergeClasses(classes.deployPanel, choice === 'api' && classes.deployPanelExpanded)}>
          ...
          <ul className={classes.preflightList}>
            ...
            <li>{strings.WelcomeDialogPreflightSignIn}</li>
            <li>{strings.WelcomeDialogPreflightRoles}</li>
          </ul>
          <Button
            appearance="transparent"
            size="small"
            className={classes.commandSwitch}
            onClick={() => {
              setCommandMode(commandMode === 'shell' ? 'powershell' : 'shell');
              setCopied(false);
            }}
          >
            {commandSwitchLabel}
          ...
          <div className={classes.codeWrap}>
            <div className={classes.codeLangBar}>
              <span className={classes.codeLangLabel}>{commandLabel}</span>
